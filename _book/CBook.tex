...
\begin{figure}
\centering
\includegraphics{01-usage_files/figure-latex/u-w-f-ex01-1.pdf}
...
用\(L(\cdot|\cdot)\)表示最优线性预测，则 \[\begin{aligned}
...
\gamma_Y(1) = 0.5,
\gamma_Y(k) = 0, k \geq 2
\end{aligned}\] 所以\(\{Y_t \}\)是一个MA(1)序列， 设其方程为 \[
...
\end{aligned}\] 即\(X_t\)是\(Y_t\)的格兰格原因。

反之， \(X_t\)是MA(1)序列， 有 \[
\eta_t = \frac{1}{1 + 0.5 B} X_t
= \sum_{j=0}^\infty (-0.5)^j X_{t-j}
\] 其中\(B\)是推移算子（滞后算子）。 于是 \[\begin{aligned}
L(X_t | \bar X_t)
=& L(\eta_t | \bar X_t)
