...
```{r, echo=FALSE}
par(family = 'SimHei')
hist(grades, breaks = 4, col = 'orange3', border='darkblue',main = '某班级《统计计算》考试成绩直方图', xlab = '成绩', ylab = '频数')
```
...
```{r}
ggplot(diamonds, aes(x = carat, y = price, color = cut, shape = color)) + geom_point() + geom_smooth()
```
...
```{r}
ggplot(diamonds) + geom_point(aes(x = carat, y = price, color = cut)) + labs(title = 'Scatterplot of carat and price', x = 'Carat', y = 'Price')
```
...
```{r}
p <- ggplot(diamonds) + geom_point(aes(x = carat, y = price, color = cut)) + labs(title = 'Scatterplot of carat and price', x = 'Carat', y = 'Price') 
p1 <- p + theme(plot.title = element_text(size=30, face="bold"), 
