...
```{r}
graddesc.lm <- function(X, y, beta.init, alpha = 1, tol = 1e-5, max.iter = 100){
  beta <- beta.init
  iter <- 1
  betas <- list()
  betas[[1]] <- beta
  while ((max(abs(l2.loss.grad(X, y, beta)))  > tol) & (iter < max.iter)){
    beta <- beta - alpha * l2.loss.grad(X, y, beta)
    iter <- iter + 1
    betas[[iter + 1]] <- beta
    # cat('迭代第',iter,'次：beta0 =', beta[1],'beta1 =',beta[2],'\n')
  }
  if (max(abs(l2.loss.grad(X, y, beta))) > tol){
    cat('算法无法收敛 \n')
  } else{
    cat('算法收敛\n')
    cat('共迭代',iter,'次','\n')
...
X <- cbind(1, x)
## Make the l2 loss function
l2.loss <- function(X, y, beta){
   m <- length(y)
   loss <- sum((X%*%beta -  y)^2)/(2*n)
return(loss)
}

l2.loss.grad <- function(X, y, beta){
  (1/n)*(t(X)%*%(X%*%beta - y))
}

gd.est <- graddesc.lm(X, y, beta.init = c(0,0), alpha = 0.1, max.iter = 1000)
```
