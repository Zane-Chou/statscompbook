...
1. $f({\mathbf x_1})\leq f({\mathbf x_r})<f({\mathbf x_n})$（${\mathbf x_r}$， 这种情况下 ${\mathbf x_r}$ 既不是最佳点也不是最差点，将 $\mathbf x_{n+1}$ 替换为 ${\mathbf x_r}$，形成新的单纯形，回到第一步。
...
3. $f({\mathbf x_r})\geq f({\mathbf x_n})$，即 ${\mathbf x_r}$ 为最差点，这意味着在 ${\mathbf x_{n+1}}$ 和 ${\mathbf x_{r}}$ 之间可能存在谷点，这时寻找收缩点（contraction point）：$${\mathbf x_c}={\mathbf x_0}+\rho({\mathbf x_{n+1}}-{\mathbf x_0})，$$通常取 $\rho=0.5$。
    - 如果 ${\mathbf x_c} < {\mathbf x_{n+1}}$，那么用收缩点 ${\mathbf x_c}$ 形成新的单纯形。
    - 如果 ${\mathbf x_c} \geq {\mathbf x_{n+1}}$，说明收缩无效，需要一个更小的单纯形，这时将 ${\mathbf x_{1}}$ 以外的点按以下方式压缩（shrink）：
...
由图 \@ref(fig:nelder) 可见，随着迭代过程，单纯形的越来越小，Nelder Mead 算法的停止条件可以是判断单纯形的体积： $$\frac{1}{2}|\mathrm{det}(\tilde{X})|，$$其中 $$\tilde{X}=\left({\mathbf x_1}-{\mathbf x_{n+1}},{\mathbf x_2-{\mathbf x_{n+1}}},\ldots, {\mathbf x_{n}-{\mathbf x_{n+1}}}\right)$$。
