...
\begin{equation}
\mathbf{x}_{n+1}=\mathbf{x}_{n}-\alpha_{n} f^{\prime}\left(\mathbf{x}_{n}\right),
(\#eq:gd)
\end{equation}

其中 $\alpha_n > 0$。如果我们的初始猜测为 $\mathbf{x}_0$，那么根据公式\@ref(eq:gd)，我们可以得到 $f(\mathbf{x}_0) \geq f(\mathbf{x}_1) \geq f(\mathbf{x}_2) \geq \cdots$，如果算法收敛，序列 $\{\mathbf{x}_n\}$ 就会收敛到 $f(\mathbf{x})$ 的局部极小值。

```{r gd, fig.cap='梯度下降算法', fig.asp=1, echo=FALSE}
...
cggplot(f_long, aes(x, y, z = z)) + 
  ...
  xlab(expression(x[1])) +
  ylab(expression(x[2])) +
  ggtitle(parse(text = paste0('"梯度下降算法： "', ' ~ f(x[1],x[2]) == ~ x[1]^2 + x[2]^2'))) 
```
...
cggplot(f_long, aes(x, y, z = z)) + 
  ...
  xlab(expression(x[1])) +
  ylab(expression(x[2])) +
  ggtitle(parse(text = paste0('"梯度下降算法： "', ' ~ f(x[1],x[2]) == ~ x[1]^2 + x[2]^2 - 1.6*x[1]*x[2]'))) 
```